...
- Manual repository sync with pagination and cached GitHub responses. Sync preserves each repository's GitHub visibility and creation date, and reads each repository's root file listing, complete GitHub language names, root `package.json`, and at most 12 immediate package manifests under `apps`, `packages`, or `services` in bounded batches inside a fixed enrichment budget, detecting a curated set of frameworks and tools and merging them with existing technology metadata. A failed or slow enrichment marks the affected repositories and still completes the import; the dashboard reports partial results. A completed sync non-destructively tombstones repositories deleted on or absent from GitHub through `github_deleted_at`, and repository-deletion webhooks mark them; marked rows are filtered from every owner, public, and AI surface while their briefs and embeddings are preserved and restored if the repository reappears. Signed webhooks update metadata for repositories already present in the database; they do not import new repositories.
...
- A privacy disclosure page (`/privacy`), a public help and FAQ guide (`/help`), an account page (`/account`) hosting the staged publication workspace and the legacy export download, and authenticated account deletion through `DELETE /api/account`, which removes the Auth user and cascades to all tenant data including the stored GitHub credential. Layout-stable loading boundaries cover dashboard, account, project, and public-profile navigation.
- Owner-curated public profiles at `/u/[slug]`. Account presents one staged publication workspace covering the profile URL, selected projects and their published brief fields, the briefing snapshot, profile state, and a preview link, deep-linking each project to its brief's publication settings. Publishing requires a saved URL and at least one non-private project selected with at least one published field; `PATCH /api/profile` enforces that server-side. Interview talking points are never publishable. The public page leads with a published snapshot of the portfolio briefing, so regenerating the private briefing never silently changes public content. Repository relationship (fork, organization-owned) is displayed separately from the owner's stated role and contributions. Private repositories are excluded unconditionally. All public data exits through one server-only retrieval boundary (`src/lib/public-profile.ts`) that returns only approved fields. Anonymous `/robots.txt` and `/sitemap.xml` expose the crawler policy, the public Help guide, and published profile slugs; unclaimed previews emit `noindex` and are excluded.
...
For an existing installation, review and apply the migrations in `supabase/migrations/` in timestamp order before deploying the corresponding application changes. This includes `20260916000000_serialize_ai_consent_embeddings.sql`, which must be applied before deploying the trigger-backed consent route, and `20260924000000_mark_deleted_github_repositories.sql`, which must be applied before deploying code that queries `projects.github_deleted_at`. Back up the database and test restoration first. Test migrations against a local or staging copy, including a second hardening-migration run to confirm idempotency. Pause writes during the hardening upgrade: it takes exclusive table locks and builds an HNSW index non-concurrently. `20260921000000_retire_legacy_features.sql` drops `projects.summary` only after copying nonblank values into `project_briefs.ai_draft`, and revokes legacy task/milestone writes without dropping their rows; because it contains a destructive column drop, review it, back up the database, and test it on local or staging PostgreSQL before any production application. On the hosted Proofstack database, `retire_legacy_features` was applied as migration version `20260922041704`: all 2 nonblank summaries were archived with matching pre/post content fingerprints, `projects.summary` is absent, zero legacy task/milestone rows changed, and the legacy tables retain only owner SELECT access. The hosted Proofstack database also records mark_deleted_github_repositories as version 20260923042352; information_schema verifies projects.github_deleted_at as a nullable timestamp with time zone.
