...
- Manual repository sync with pagination and cached GitHub responses. Sync also reads each repository's root `package.json` in bounded batches, detects a curated set of frameworks and tools from dependency names, and merges those values with existing technology metadata. Signed webhooks update metadata for repositories already present in the database; they do not import new repositories or automatically regenerate summaries.
- The project-processing API supports README summaries and supplemental technology extraction using `gemini-3.5-flash`, falling back to `gemini-3.1-flash-lite`; summary generation is not exposed as a dashboard action. Chat uses the same model pair through `@google/genai`.
...
- Hybrid chat always supplies up to 500 authenticated-user project records as structured metadata, including a normalized technology-to-project index, and retrieves up to five matching README documents for semantic detail. Technology punctuation variants such as Next.js and NextJS are grouped together. Indexed technology lists can remain incomplete when frameworks are absent from the root manifest and README, so the assistant reports a lack of indexed evidence rather than claiming that no project uses an unlisted technology. The context reports whether the catalog is complete so the model does not make unsupported exhaustive claims. Project-scoped requests filter both sources to the selected project, and structured metadata remains available if embedding or vector retrieval is unavailable. Chat does not browse source files, execute code, or retrieve task/milestone state. A code-map helper exists, but no code-map upload/indexing workflow is exposed.
- Searchable and sortable dashboard, task/milestone tracking, locally bundled Devicons, and keyboard-accessible controls.
- Responsive console-ledger styled dark interface (Space Grotesk / Space Mono, phosphor accent), GitHub Linguist language colors, signed-in user identity with sign-out in the sticky header, a viewport-safe floating chat panel, and a layered-squares logo served as the app icon.
